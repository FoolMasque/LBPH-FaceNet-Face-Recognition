...
img = cv2.imread('face_dataset/ellen-degeneres.jpg')
...
for rectangle in rectangles:
    if rectangle is not None:
        ...
        cv2.imshow("after align",new_img)
        ...
        # crop_img = img[int(rectangle[1]):int(rectangle[3]), int(rectangle[0]):int(rectangle[2])]
        # crop_img = cv2.resize(crop_img,(160,160))
        # new_img,_ = func.Alignment_2(crop_img,std_landmark,landmark)
        # cv2.imshow("affine",new_img)    
        new_img = np.expand_dims(new_img,0)
        feature1 = func.calc_128_vec(facenet_model,new_img)
        print(feature1)
